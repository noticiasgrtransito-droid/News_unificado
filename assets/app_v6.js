/* CON_News v6.0 unified intelligent fetcher (client-side) */
(async function(){
  ...
  function detectType(text){
    ...
    if(/interdi|bloqueio|obra|manuten|lento|lentidao|lentidão|trânsito/i.test(t)) return 'Interdição';
  ...
  async function fetchFeeds(){
    ...
    for(const q of KEYWORD_QUERIES){
      try{ const j = await rss2jsonFetch(googleNewsRss(q)); if(j && j.items) j.items.forEach(it=> collected.push({ title: it.title, link: it.link, pubDate: it.pubDate || it.isoDate, source: 'GoogleNews', snippet: (it.description||'').replace(/<[^>]+>/g,'') })); }catch(e){ console.warn('google news q fail', q, e); }
    }
    const map = new Map();
    collected.forEach(it=>{ const key = (it.link || it.title).trim(); if(!map.has(key)) map.set(key, it); });
    const items = Array.from(map.values()).map(it=>{ it.type = detectType(it.title + ' ' + (it.snippet||'')); it.road = detectRoad(it.title + ' ' + (it.snippet||'')); it.region = detectRegion(it.title + ' ' + (it.snippet||'')); return it; }).sort((a,b)=> (new Date(b.pubDate).getTime()||0)-(new Date(a.pubDate).getTime()||0));
    localStorage.setItem('congrl_cache', JSON.stringify({ items: items, fetched: new Date().toISOString() })); localStorage.setItem('congrl_used_sources', JSON.stringify(Array.from(new Set(items.map(i=>i.source||'local'))))); return items;
  }

  async function loadAndRender(){ allFeeds = await fetchFeeds(); document.getElementById('lastFetch').textContent = new Date().toLocaleString(); applyFilters(); }
  ...
  function applyFilters(){
    ...
    renderNews(filtered); updateStatsAndCharts(filtered); updateMapMarkers(filtered);
  }
  ...
  function renderNews(list){
    ...
    list.forEach(it=>{
      ...
      div.innerHTML = `<div style="flex:1;min-width:220px"><span class="meta">[${it.type}] ${it.road? '— '+it.road : ''}</span> ${link}</div><div class="meta">${escapeHtml(it.source||'')} • ${timestr}</div>`;
  ...
  function updateStatsAndCharts(list){
    ...
    list.forEach(f=>{ typesCount[f.type] = (typesCount[f.type]||0) + 1; regionsCount[f.region] = (regionsCount[f.region]||0) + 1; });
